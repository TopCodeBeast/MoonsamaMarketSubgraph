...
export function onFill(event: FillEvent): void {
    ...
    collectionStat.lastTrade = ltp.id

    //log.warning("as-highestsell: {}", [assetStat.highestSellEver])
    //log.warning("as-highestbuy: {}", [assetStat.highestBuyEver])
    //log.warning("cs-highestsell: {}", [collectionStat.highestSellEver])
    //log.warning("cs-highestbuy: {}", [collectionStat.highestBuyEver])
...
export function onPaymentToken(event: PaymentTokenEvent): void {
    let id = event.params.token.toHexString()

    let assetId = id.concat("-0")
    let asset = Asset.load(assetId)

    if (asset == null) {
        asset = new Asset(assetId)
        asset.assetId = BigInt.fromI32(0)
        asset.assetType = toAssetType(BigInt.fromI32(1))
        asset.assetAddress = id
        asset.save()
    }

    let ptoken = PaymentToken.load(id)
    if (ptoken == null) {
        ptoken = new PaymentToken(id)
        ptoken.asset = assetId
        ptoken.totalVolume = BigInt.fromI32(0)
    }

    ptoken.enabled = event.params.enabled
    ptoken.save()
}
